...
class Quantity:
    def __init__(self, value: float | int, unit: Unit):
        if not isinstance(unit, Unit):
            raise TypeError(
                f"Cannot create a Quantity. 'unit' must be a Unit object, not {type(unit)}."
            )
    ...
    def to(self, new_unit: Unit) -> Quantity:
        if not isinstance(new_unit, Unit):
            raise TypeError(
                f"Cannot convert. 'new_unit' must be a Unit object, not {type(new_unit)}."
            )
        if self.unit.dimension != new_unit.dimension:
            raise TypeError(
                f"Cannot convert from dimension {self.unit.dimension!r} to {new_unit.dimension!r}"
            )
    ...
    def __sub__(self, other: Quantity) -> Quantity:
        ...
        if self.unit.dimension != other.unit.dimension:
            raise TypeError(
                f"Cannot subtract {other.unit.dimension!r} from {self.unit.dimension!r}"
            )
        new_base_value = self.base_value - other.base_value
        return Quantity(new_base_value / self.unit.factor, self.unit)
